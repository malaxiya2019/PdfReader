...
///
/// 性能优化 (Phase 9):
/// - Isolate 处理大文件 I/O（读取/写入分离到后台线程）
/// - Pre-check 文件大小，超大文件提前警告
/// - 用完即释放 PdfDocument 资源
class PdfToolService {
  PdfToolService._();

  /// 大文件阈值（超过此值建议使用 Isolate）
  ...
  static Future<PdfToolResult> pdfToImages({
    ...
    try {
      ...
      return PdfToolResult(
        success: true,
        message: '已导出 $totalPages 页为 $imageFormat 格式',
        outputPath: outputDir,
      );
    } catch (e) {
  ...
  static Future<PdfToolResult> rotatePages({
    ...
    required String outputPath,
    required List<int> pageNumbers,
    required int rotation, // 90, 180, 270
  }) async {
    try {
      final bytes = await _readBytesInIsolate(inputPath);
      final doc = PdfDocument(inputBytes: bytes);
